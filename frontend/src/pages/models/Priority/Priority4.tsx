...
export const Priority4: React.FC = () => {
  const [inputs, setInputs] = useState<PriorityComInput>({
    s: '',
    mu: '',
    lambdas: ['', ''],
  });
  ...
  const handleCalculate = async (e: React.FormEvent) => {
    ...
    // Validações
    if (!s || s <= 1) {
      setError('Número de servidores (s) deve ser maior que 1 (use Priority3 para s=1)');
      return;
    }
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="mb-6">
        <h2 className="text-4xl font-display font-bold text-dark-950 mb-2">
          M/M/S com Prioridade (COM Interrupção)
        </h2>
        <p className="text-dark-600 text-lg">
          Múltiplas Filas • <strong>2 ou Mais Servidores</strong> • Prioridade Preemptiva
        </p>
      </div>
          ...
          <div>
            ...
            <div className="text-red-900">
              ...
              <div className="mt-2 bg-blue-100 border border-blue-400 p-2 rounded text-sm">
                <strong>ℹ️ Para 1 servidor (s=1):</strong> Use o modelo <strong>Priority3</strong>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-gradient-to-br from-red-500 to-red-700 rounded-2xl shadow-2xl p-1 mb-6">
        <div className="bg-white rounded-xl p-6">
          <h3 className="font-display font-bold text-red-900 text-2xl mb-1">
            🧮 Calculadora M/M/S com Prioridade Preemptiva (s ≥ 2)
          </h3>
          ...
          <form onSubmit={handleCalculate} className="space-y-5">
              ...
              <div className="bg-blue-50 rounded-lg p-4 border-2 border-blue-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-sm">
                  <p>
                    ...
                    {(() => {
                      const lambdaTotal = inputs.lambdas.reduce<number>(
                        (sum, l) => sum + Number(l || 0),
                        0
                      );
                      return lambdaTotal.toFixed(4);
                    })()}
                  </p>
                  <p>
                    <strong>Capacidade (s×μ):</strong>{' '}
                    {(() => {
                      const sNum = Number(inputs.s || 0);
                      const muNum = Number(inputs.mu || 0);
                      return (sNum * muNum).toFixed(4);
                    })()}
                  </p>
                  <p>
                    ...
                    {(() => {
                      ...
                      const sNum = Number(inputs.s || 0);
                      const muNum = Number(inputs.mu || 0);
                      const capacity = sNum * muNum;
                      return capacity === 0 ? '0.0000' : (lambdaTotal / capacity).toFixed(4);
                    })()}
                  </p>
                </div>
              </div>
            ...
            <Button type="submit" fullWidth>
              🚀 CALCULAR COM PRIORIDADE PREEMPTIVA (s ≥ 2)
            </Button>
          </form>
      ...
      <div className="bg-gradient-to-br from-indigo-50 to-indigo-100 border-2 border-indigo-400 rounded-xl p-5 shadow-lg">
        <h4 className="font-bold text-indigo-900 text-xl mb-4">📐 Fórmulas do Modelo Preemptivo (M/M/S, s≥2)</h4>
